...
MODE_MAP = {"daytrade": "當沖", "overnight": "隔日沖", "weekly": "週沖"}
WATCH_MAP = {
...
def write_excel_report(output_dir, snapshot_rows, groups, overlap_rows, excluded):
    wb = Workbook()
    wb.remove(wb.active)
    ...
    target = output_dir / "stock_screen_report.xlsx"
    try:
        wb.save(target)
    except PermissionError:
        fallback = output_dir / f"stock_screen_report_{datetime.now():%H%M%S}.xlsx"
        wb.save(fallback)
